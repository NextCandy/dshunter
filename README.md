...
- 🌐 **多注册商域名同步**：Spaceship、Dynadot、Porkbun、Namecheap、阿里云、腾讯云、西部数码、Cloudflare Registrar，一键 `同步注册商域名`，结果写入后端持久化文件，刷新、重新登录、容器重启后不丢失。
...
注册商域名同步结果由后端写入挂载卷 `data/registrar-domains.json`，包含域名、来源注册商、首次发现时间、最近同步时间、NS 状态和最近同步任务记录。`/domains` 页面刷新时会先从后端恢复这些记录；再次同步同一注册商会更新已有域名并标记注册商本次未返回的域名，不会直接删除。CSV 导出使用 UTF-8 BOM，方便 Excel/WPS 直接打开中文列名。

同步相关 API：

- `POST /api/registrars/:id/sync-domains`
- `GET /api/registrar-domains`
- `GET /api/registrar-sync-jobs`
- `GET /api/registrar-domains/:id`
- `PATCH /api/registrar-domains/:id`
