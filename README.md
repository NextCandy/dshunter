...
- **公开首页 `/`** — 无需登录的域名展示橱窗：紧凑筛选区（名称搜索、后缀、主体位数、精品、分类，选项均从真实数据自动生成）+ 响应式域名网格（2–6 列自适应）+ 分页，桌面端一屏内完成不整页滚动；**精品域名**带星标强调并始终置顶，组内按排序权重与字母序排列；页脚联系方式为小图标超链接（后台可配 email / Telegram / 微信 / X / GitHub / 自定义）。
...
- **完整信息编辑** — 每个手动域名可编辑注册商、DNS 修改地址、NS 服务器、注册/到期日期、备注、标签、分组，以及前台展示用的**精品标记 / 分类 / 排序权重**。
...
截至 2026-07-09，主分支已完成站点设置、动态注册商、注册商同步预检、全量 lint 门禁、API 类型收紧、Fast Refresh lint 清零、构建弃用提示清理、登录页站点品牌/资产读数动态化，以及**首页域名网格改版**（筛选 + 分页 + 精品/分类/排序权重 + 页脚联系图标 + 全链路域名去重 + 新品牌 Logo/favicon，commit `17b264c`，已部署 NAS 并线上验证）。最新阶段记录、NAS 部署状态、验证证据和回滚提示见 [`PROGRESS.md`](./PROGRESS.md)。
